...
for algo in ["ANN","KNN","LR","SVM"]:
    for k in range(0,5):
        ...
        if algo == "ANN":
            stack_model = MLPClassifier(hidden_layer_sizes=(101+1), max_iter=1000)
            base_models = [MLPClassifier(hidden_layer_sizes=(101+1), max_iter=1000) for _ in range(0, model_measures.shape[0])]
            
        elif algo == "KNN":
            stack_model = KNeighborsClassifier(n_neighbors=5)
            base_models = [KNeighborsClassifier(n_neighbors=5) for _ in range(0, model_measures.shape[0])]
            
        elif algo == "LR":
            stack_model = LogisticRegression(max_iter=1000)
            base_models = [LogisticRegression(max_iter=1000) for _ in range(0, model_measures.shape[0])]
            
        elif algo == "SVM":
            stack_model = SVC(tol=0.001,probability=True)
            base_models = [SVC(tol=0.001,probability=True) for _ in range(0, model_measures.shape[0])]
        ...
        model_report.to_csv("Outputs/Raw/{}/{}.csv".format(algo,str(k+1)))
...
for algo in ["ANN","KNN","LR","SVM"]:
    for k in range(0,5):
        ...
        if algo == "ANN":
            stack_model = MLPClassifier(hidden_layer_sizes=(5*21+1), max_iter=1000)
            base_models = [MLPClassifier(hidden_layer_sizes=(10+1), max_iter=1000) for _ in range(0, model_measures.shape[0])]
        ...
        model_report = pd.DataFrame(np.column_stack((np.array(y_test),y_pred)),columns=["y_true","y_pred"])
        model_report.to_csv("Outputs/AE/{}/{}.csv".format(algo,str(k+1)))
...
for algo in ["SVM"]:
    for k in range(0,5):
        ...
        if algo == "ANN":
            anomaly_detection_model = MLPClassifier(hidden_layer_sizes=(21+1), max_iter=1000)
                
        elif algo == "KNN":
            anomaly_detection_model = KNeighborsClassifier(n_neighbors=5)
            
        elif algo == "LR":
            anomaly_detection_model = LogisticRegression(max_iter=1000)
            
        elif algo == "SVM":
            anomaly_detection_model = SVC(tol=0.001,probability=True)
        ...
        model_report = pd.DataFrame(np.column_stack((np.array(y_test),y_pred)),columns=["y_true","y_pred"])
        model_report.to_csv("Outputs/Anomaly/{}/{}.csv".format(algo,str(k+1)))